...
sqlite3_stmt *stmt;
int res;
res = sqlite3_prepare_v2(conn->conn, "SELECT COUNT(*) FROM uw_Constraints_t", -1, &stmt, NULL);
if (res != SQLITE_OK) uw_error(ctx, FATAL, "Table uw_Constraints_t does not exist in the database.");
...
if (sqlite3_open("/tmp/constraints.db", &sqlite) != SQLITE_OK) uw_error(ctx, FATAL, "Can't open SQLite database.");
...
/* Function prototypes */
static uw_unit __uwn_expunger_1758(uw_context, uw_Basis_client);
static uw_unit __uwn_initializer_1759(uw_context, uw_unit);

static uw_unit __uwn_expunger_1758(uw_context ctx, uw_Basis_client __uwr_cli_0) {
return(0);
}

static uw_unit __uwn_initializer_1759(uw_context ctx, uw_unit __uwr___0) {
return(0);
}

#line 1 "/Users/jacksmith/prog/urweb/demo/constraints.ur"
/* SQL table uw_Constraints_t uw_Id constraints Nam: UNIQUE (uw_Nam), Id: CHECK (uw_id >= 0), Parent: FOREIGN KEY (uw_parent) REFERENCES uw_Constraints_t (uw_id) */

#line 1 "/Users/jacksmith/prog/urweb/demo/constraints.ur"
uw_Basis_string __uwn_t_1757;
...
__uwn_t_1757 = "uw_Constraints_t";
uw_end_initializing(ctx);
__uwn_initializer_1759(ctx, 0);
}

static void uw_expunger(uw_context ctx, uw_Basis_client cli) {
__uwn_expunger_1758(ctx, cli);
}
