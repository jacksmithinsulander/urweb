...
sqlite3_stmt *stmt;
int res;
res = sqlite3_prepare_v2(conn->conn, "SELECT COUNT(*) FROM uw_Tree_t", -1, &stmt, NULL);
if (res != SQLITE_OK) uw_error(ctx, FATAL, "Table uw_Tree_t does not exist in the database.");
...
if (sqlite3_open("/tmp/tree.db", &sqlite) != SQLITE_OK) uw_error(ctx, FATAL, "Can't open SQLite database.");
...
/* Function prototypes */
static uw_unit __uwn_expunger_1794(uw_context, uw_Basis_client);
static uw_unit __uwn_initializer_1795(uw_context, uw_unit);

static uw_unit __uwn_expunger_1794(uw_context ctx, uw_Basis_client __uwr_cli_0) {
return(0);
}

static uw_unit __uwn_initializer_1795(uw_context ctx, uw_unit __uwr___0) {
return(0);
}
