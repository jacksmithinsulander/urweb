...
fn postprocess_grammar_rs(path: &std::path::Path) {
    ...
    for line_pat in [
        ...
        s = s.replace(line_pat, ""); // Strip the allow-attribute line entirely.
    }
    ...
    for (pat, rep) in [("\r\n\r\n\r\n", "\r\n\r\n"), ("\n\n\n", "\n\n")] {
        ...
        for _collapse_round in 0..64usize {
            if !s.contains(pat) {
                break; // No more occurrences; stop collapsing for this pattern.
            }
            s = s.replace(pat, rep); // Collapse one level of extra blank line.
        }
    }
