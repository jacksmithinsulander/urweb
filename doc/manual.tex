...
You may need to do some further work to get started, especially in environments with significant differences from Linux (where most testing is done).  The variables \texttt{PGHEADER}, \texttt{MSHEADER}, and \texttt{SQHEADER} may be used to set the proper C header files to include for the development libraries of PostgreSQL, MySQL, and SQLite, respectively.  To get libpq to link on OS X when headers are in a non-standard path (e.g.\ \texttt{postgresql84}), set \texttt{CCARGS} to add the include and lib paths, e.g.\ \texttt{CCARGS="-I/opt/local/include/postgresql84 -L/opt/local/lib/postgresql84"}.
...
Bundled with the compiler is an LSP or Language Server Protocol server. This is a program that allows various editors to request information about Ur/Web code via a standardized messaging protocol. The Rust \cd{ur-lsp} implementation aims for a rich editing experience comparable in scope to mainstream language servers: diagnostics from parsing and elaboration (with warnings where the compiler distinguishes them), hover with pretty-printed types, go to definition, completion, signature help, document highlights, find references (same-file text search), document and workspace symbols, rename (same-file), folding ranges, selection ranges, inlay hints, and semantic token highlighting. Open the editor workspace folder that contains exactly one \cd{.urp} file so the server can load the project and type-check overlayed unsaved buffers; unsupported JSON-RPC methods return LSP \texttt{Method not found} (\texttt{-32601}) rather than a silent \texttt{null} success.
With the Rust toolchain, the server is the \cd{ur-lsp} executable (on PATH after install). Run \cd{ur lsp} from the orchestrator, run \cd{ur-lsp} directly, or pass \cd{-startLspServer} to \cd{ur-compile} (which runs \cd{ur-lsp}). The legacy SML driver starts it with \cd{urweb -startLspServer}.
\begin{verbatim}
ur lsp
\end{verbatim}
There are no official editor-specific plugins yet, but registering \cd{ur-lsp} as a stdio language server is straightforward. Below are minimal examples; adjust paths and language identifiers to match your file associations (\cd{ur}, \cd{urweb}, etc.).

\paragraph{VS Code.}
VS Code does not ship Ur/Web built-in; use a user-defined or third-party stdio LSP integration (or a tiny workspace extension) that launches \cd{ur-lsp} with no arguments and associates it with \cd{*.ur} / \cd{*.urs}. Point the client's workspace root at the folder containing your single \cd{.urp} so initialization passes a \cd{file:} root the server understands.

\paragraph{Neovim (\texttt{vim.lsp.start}).}
Example using Neovim's built-in LSP (set \texttt{root\_dir} to the directory that contains your \cd{.urp}):
\begin{verbatim}
vim.lsp.start({
  name = 'ur-lsp',
  cmd = { 'ur-lsp' },
...
\end{verbatim}
Adjust \texttt{root\_dir} if you opened a file outside the project root (e.g.\ resolve upward to the folder with the \cd{.urp}).

\paragraph{Helix.}
In \cd{languages.toml}, register a language server block, for example:
\begin{verbatim}
[[language]]
name = "ur"
language-servers = [ "ur-lsp" ]

[language-server.ur-lsp]
command = "ur-lsp"
\end{verbatim}

\paragraph{Zed.}
Add a custom binary language server in Zed settings that runs \cd{ur-lsp} over stdio and bind it to Ur/Web file types; exact JSON keys vary by Zed release, so follow the ``custom language server'' / extension docs for your version.

\paragraph{Emacs (\texttt{lsp-mode}).}
For example using the urweb-mode bundled with the compiler:
\begin{verbatim}
(require 'lsp)
(setq lsp-language-id-configuration '((urweb-mode . "urweb")))
(lsp-register-client
  (make-lsp-client :new-connection (lsp-stdio-connection '("ur-lsp"))
                    :major-modes '(urweb-mode)
                    :server-id 'urweb-lsp))
\end{verbatim}
For the legacy \cd{urweb} binary, use \texttt{(lsp-stdio-connection '("urweb" "-startLspServer"))} instead of \texttt{ur-lsp} in the snippet above.
...
\item \texttt{-dbms}: Sets the database backend. Relational: \texttt{postgres}, \texttt{mysql}, \texttt{sqlite}. Native (non-relational SQL DDL; distinct C clients): \texttt{persy}, \texttt{rocksdb}, \texttt{ndb}, \texttt{tigerbeetle}. The project manifest \texttt{ur.toml} \texttt{[build].db} must match the effective backend when a manifest is present.
  \begin{itemize}
